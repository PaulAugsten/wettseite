...
- `react-doctor/js-set-map-lookups` - scraper files (match_parser.ts, matches_scraper.ts):
  flagged `.includes()` calls are substring tests on strings (`stage`, `currentRound`,
  `tournament.url`), not array scans. Skip after verifying the receiver is a string.

- `react-doctor/async-await-in-loop` - Liquipedia scraper loops (matches_scraper.ts,
  tournaments_scraper.ts): serial awaits with 2-5s sleeps are
